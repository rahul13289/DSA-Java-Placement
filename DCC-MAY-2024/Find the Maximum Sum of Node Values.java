...
Approach
Detailed Approach Explained in Video Here
Initialization:
...
class Solution {
    public long maximumValueSum(int[] nums, int k, int[][] edges) {
     ...
     int positive_min = Integer.MAX_VALUE;
     int negative_max = Integer.MIN_VALUE;

     for(int nodeValue : nums){

        int node_value_after_operation = nodeValue ^ k;
        totalsum += nodeValue;

        int net_change = node_value_after_operation - nodeValue;

        if(net_change > 0){

            positive_min = Math.min(positive_min , net_change);
            totalsum += net_change;
            count += 1;

        }else{

            negative_max = Math.max(negative_max , net_change);
     ...
     return Math.max(totalsum - positive_min , totalsum + negative_max);
